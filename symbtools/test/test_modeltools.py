...
class ModelToolsTest(unittest.TestCase):
    ...
    def test_equilibrium_one(self):
        ...
        T = 1 / 2 * (m1 * tthetad[0] ** 2 + m2 * tthetad[1] ** 2)  # total kinetic energy
        # total potential energy
        V = 1 / 2 * (k1 * ttheta[0] ** 2 + k2 * ttheta[1] ** 2 + k3 * (ttheta[1] - ttheta[0]) ** 2)
        external_forces = [0, F]
        ...
        eqrt_point = list(zip(mod.tt, mod.eqlbr))  # equilibrium points of states
    ...
    def test_equilibrium_all(self):
        mod = mt.SymbolicModel()
        Np = 2  # number of active coordinates
        pp = st.symb_vector("p1:{0}".format(Np + 1))  # active coordinates
        mod.tt = ttheta = pp  # position of the bodies in m
        mod.ttd = st.time_deriv(ttheta, ttheta)  # velocities of the bodies in m/s
        mod.ttdd = st.time_deriv(ttheta, ttheta, order=2)  # accelerations in m/s^2
        mod.eqns = sp.Matrix([[ttheta[0] - ttheta[1]], [3 * ttheta[1] * (1 - ttheta[0] ** 2)]])

        # calculate equilibrium point
        mod.calc_eqlbr(etype='all_ep')

        for i in mod.eqlbr:
            eqrt_point = list(zip(mod.tt, list(i)))  # equilibrium points of states
            state_eqns = mod.eqns.subz0(mod.ttd, mod.ttdd).subs(eqrt_point)
            for j in range(len(mod.tt)):
                self.assertAlmostEqual(0, state_eqns[j], delta=6)
    ...
    def test_four_bar_constraints(self):
        ...
        parameter_values = dict(s1=1 / 2, s2=1 / 2, s3=1 / 2, m1=1, m2=1, m3=3, J1=1 / 12, J2=1 / 12, J3=1 / 12, l1=1,
                                l2=1.5,
    ...
    def test_triple_pendulum(self):
        ...
        eqns_ref = Matrix([[J3 * (2 * p1_dd + 2 * q1_dd + 2 * q2_dd) / 2 + g * m3 * s3 * sin(p1 + q1 + q2) + m3 * s3 * (
                    -l1 * q1_d ** 2 * sin(q1) + l1 * q1_dd * cos(q1) - l2 * (q1_d + q2_d) ** 2 * sin(q1 + q2) + l2 * (
                        q1_dd + q2_dd) * cos(q1 + q2) - s3 * (p1_d + q1_d + q2_d) ** 2 * sin(p1 + q1 + q2) + s3 * (
                                p1_dd + q1_dd + q2_dd) * cos(p1 + q1 + q2)) * cos(p1 + q1 + q2) + m3 * s3 * (
                                        l1 * q1_d ** 2 * cos(q1) + l1 * q1_dd * sin(q1) + l2 * (q1_d + q2_d) ** 2 * cos(
                                    q1 + q2) + l2 * (q1_dd + q2_dd) * sin(q1 + q2) + s3 * (
                                                    p1_d + q1_d + q2_d) ** 2 * cos(p1 + q1 + q2) + s3 * (
                                                    p1_dd + q1_dd + q2_dd) * sin(p1 + q1 + q2)) * sin(p1 + q1 + q2)], [
                               J1 * q1_dd + J2 * (2 * q1_dd + 2 * q2_dd) / 2 + J3 * (
                                           2 * p1_dd + 2 * q1_dd + 2 * q2_dd) / 2 - Q1 + g * m1 * s1 * sin(q1) + \
                               g * m2 * (l1 * sin(q1) + s2 * sin(q1 + q2)) + g * m3 * (
                                           l1 * sin(q1) + l2 * sin(q1 + q2) + s3 * sin(
                                       p1 + q1 + q2)) + m1 * q1_dd * s1 ** 2 * sin(
                                   q1) ** 2 + m1 * q1_dd * s1 ** 2 * cos(q1) ** 2 + m2 * (
                                           2 * l1 * sin(q1) + 2 * s2 * sin(q1 + q2)) * (
                                           l1 * q1_d ** 2 * cos(q1) + l1 * q1_dd * sin(q1) + s2 * (
                                               q1_d + q2_d) ** 2 * cos(q1 + q2) + s2 * (q1_dd + q2_dd) * sin(
                                       q1 + q2)) / 2 + m2 * (2 * l1 * cos(q1) + 2 * s2 * cos(q1 + q2)) * (
                                           -l1 * q1_d ** 2 * sin(q1) + l1 * q1_dd * cos(q1) - s2 * (
                                               q1_d + q2_d) ** 2 * sin(q1 + q2) + s2 * (q1_dd + q2_dd) * cos(
                                       q1 + q2)) / 2 + m3 * (
                                           2 * l1 * sin(q1) + 2 * l2 * sin(q1 + q2) + 2 * s3 * sin(p1 + q1 + q2)) * (
                                           l1 * q1_d ** 2 * cos(q1) + l1 * q1_dd * sin(q1) + l2 * (
                                               q1_d + q2_d) ** 2 * cos(q1 + q2) + l2 * (q1_dd + q2_dd) * sin(
                                       q1 + q2) + s3 * (p1_d + q1_d + q2_d) ** 2 * cos(p1 + q1 + q2) + s3 * (
                                                       p1_dd + q1_dd + q2_dd) * sin(p1 + q1 + q2)) / 2 + m3 * (
                                           2 * l1 * cos(q1) + 2 * l2 * cos(q1 + q2) + 2 * s3 * cos(p1 + q1 + q2)) * (
                                           -l1 * q1_d ** 2 * sin(q1) + l1 * q1_dd * cos(q1) - l2 * (
                                               q1_d + q2_d) ** 2 * sin(q1 + q2) + l2 * (q1_dd + q2_dd) * cos(q1 + q2) - \
                                           s3 * (p1_d + q1_d + q2_d) ** 2 * sin(p1 + q1 + q2) + s3 * (
                                                       p1_dd + q1_dd + q2_dd) * cos(p1 + q1 + q2)) / 2], [
                               J2 * (2 * q1_dd + 2 * q2_dd) / 2 + J3 * (
                                           2 * p1_dd + 2 * q1_dd + 2 * q2_dd) / 2 - Q2 + g * m2 * s2 * sin(
                                   q1 + q2) + g * m3 * (l2 * sin(q1 + q2) + s3 * sin(p1 + q1 + q2)) + m2 * s2 * (
                                           -l1 * q1_d ** 2 * sin(q1) + l1 * q1_dd * cos(q1) - s2 * (
                                               q1_d + q2_d) ** 2 * sin(q1 + q2) + s2 * (q1_dd + q2_dd) * cos(
                                       q1 + q2)) * cos(q1 + q2) + \
                               m2 * s2 * (l1 * q1_d ** 2 * cos(q1) + l1 * q1_dd * sin(q1) + s2 * (
                                           q1_d + q2_d) ** 2 * cos(q1 + q2) + s2 * (q1_dd + q2_dd) * sin(
                                   q1 + q2)) * sin(q1 + q2) + m3 * (
                                           2 * l2 * sin(q1 + q2) + 2 * s3 * sin(p1 + q1 + q2)) * (
                                           l1 * q1_d ** 2 * cos(q1) + l1 * q1_dd * sin(q1) + l2 * (
                                               q1_d + q2_d) ** 2 * cos(q1 + q2) + l2 * (q1_dd + q2_dd) * sin(
                                       q1 + q2) + s3 * (p1_d + q1_d + q2_d) ** 2 * cos(p1 + q1 + q2) + s3 * (
                                                       p1_dd + q1_dd + q2_dd) * sin(p1 + q1 + q2)) / 2 + m3 * (
                                           2 * l2 * cos(q1 + q2) + 2 * s3 * cos(p1 + q1 + q2)) * (
                                           -l1 * q1_d ** 2 * sin(q1) + l1 * q1_dd * cos(q1) - l2 * (
                                               q1_d + q2_d) ** 2 * sin(q1 + q2) + l2 * (q1_dd + q2_dd) * cos(
                                       q1 + q2) - s3 * (p1_d + q1_d + q2_d) ** 2 * sin(p1 + q1 + q2) + s3 * (
                                                       p1_dd + q1_dd + q2_dd) * cos(p1 + q1 + q2)) / 2]])
